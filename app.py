...
FlaskJSON(app)
api = Api(app, title='SCD-UCA Middleware Koha-Primo', api_version='1.0', api_spec_url='/api/swagger', base_path=f'{host}:{port}{url_subpath}')
...
class KohaApiPubliqueItems(Resource):
    ...
    def get(self, id):
        url = f"{koha_api_public_test}biblios/{id}/items"
        response = requests.request("GET", url).text
        data = json.loads(response)        
        return jsonify(data)

api.add_resource(HelloWorld, f'/api/{api_version}', f'/api/{api_version}/hello')      
api.add_resource(KohaApiPubliqueItems, f'/api/{api_version}/koha_items/<string:id>')
